...
from riftcheck.topology import topology_summary
...
def export_mast(trace: Trace, path: Path) -> None:
    ...
    and categorisation tools on their Riftcheck traces.
    ...
    with path.open("w") as fh:
        ...
        header = {
            ...
            "riftcheck_version": _get_version(),
...
def export_html(
    ...
    ``window.__RIFTCHECK_DATA__``. The output is a single portable file that
...
def dashboard_html(
    ...
    for name, trace in traces:
        ...
        from riftcheck.engine import wilson_interval
...
def _write_viewer_html(data: dict, title: str, path: Path) -> None:
    ...
    ``window.__RIFTCHECK_DATA__`` immediately before ``</head>`` so the React
    ...
    injection = f"<script>window.__RIFTCHECK_DATA__={payload};</script>"
