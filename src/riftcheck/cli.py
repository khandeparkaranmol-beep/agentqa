...
from riftcheck import __version__


@click.group()
@click.version_option(version=__version__, prog_name="riftcheck")
def main() -> None:
    """Riftcheck — multi-agent interaction testing framework."""
...
@click.option("--save-traces", is_flag=True, help="Save JSONL traces to .riftcheck/traces/.")
...
def run(
    ...
    for scenario_path in scenario_paths:
        ...
        from riftcheck.display import print_trace, print_summary
        ...
        if save_traces or open_viewer:
            trace_dir = Path(".riftcheck") / "traces"
...
@click.option("--title", default="Riftcheck Trace", show_default=True, help="Title for HTML exports.")
def export(trace_path: str, fmt: str, output: str | None, title: str) -> None:
    ...
    from riftcheck.export import export_html, export_mast
...
def diff_cmd(
    ...
    import webbrowser
    from riftcheck.trace import Trace
    from riftcheck.export import diff_html
...
@click.option("--output", "-o", default=None, type=click.Path(), help="Output HTML file.")
@click.option("--title", default="Riftcheck Dashboard", show_default=True)
@click.option("--no-open", is_flag=True, help="Write the file but do not open it in a browser.")
def dashboard(directory: str, output: str | None, title: str, no_open: bool) -> None:
    ...
    import webbrowser
    from riftcheck.trace import Trace
    from riftcheck.export import dashboard_html
...
def init(
    ...
    \b
    Examples:
      riftcheck init                       # scan current directory
      riftcheck init ./my_project          # scan a specific directory
      riftcheck init . --framework crewai  # force CrewAI scanner
      riftcheck init . -o tests/           # write files to tests/
    """
    if verbose:
        logging.basicConfig(level=logging.DEBUG)

    from riftcheck.scanner import scan_project
    ...
    if result is None:
        click.echo(
            ...
            "See: riftcheck run examples/annotated/01_getting_started.yaml --view",
            err=True,
        )
        sys.exit(1)
    ...
    click.echo(f"     {click.style(f'riftcheck run {scenario_path} --view', fg='cyan')}\n")
...
def _load_agents(agents_path: Path) -> dict:
    ...
    spec = importlib.util.spec_from_file_location("_riftcheck_agents", agents_path)
