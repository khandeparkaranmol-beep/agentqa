...
from riftcheck.scenario import ScenarioConfig, load_scenario


def pytest_collect_file(parent: pytest.Collector, file_path: Path) -> pytest.Collector | None:
    """Collect .yaml files that contain an 'agents' key as Riftcheck test items."""
...
class ScenarioItem(pytest.Item):
    """A single pytest test item representing one Riftcheck scenario."""
    ...
    def runtest(self) -> None:
        ...
        if failures:
            raise RiftcheckAssertionError("\n".join(failures))

    def repr_failure(self, excinfo: Any) -> str:  # type: ignore[override]
        if isinstance(excinfo.value, RiftcheckAssertionError):
            return f"Riftcheck scenario FAILED:\n{excinfo.value}"
        return super().repr_failure(excinfo)

    def reportinfo(self) -> tuple[Any, int | None, str]:
        return self.path, None, f"riftcheck: {self.scenario.name}"


class RiftcheckAssertionError(Exception):
