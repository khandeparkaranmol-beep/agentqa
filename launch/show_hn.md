...
### Title
Show HN: Riftcheck – Property-based testing for multi-agent AI systems

### Body

I built Riftcheck because I kept hitting bugs in multi-agent systems that no single-agent test catches — information leaking between agents, deadlocks, agents looping the same response, tasks marked done before completion. These only surface when agents interact.
...
`riftcheck init` AST-scans your Python project (no LLM), detects agents from CrewAI, LangGraph, AutoGen, or plain callables, and generates a scenario.yaml + agents.py with fault injection and property assertions.
...
### Title
Show HN: Riftcheck – 16 property checkers for multi-agent AI bugs (Python, MIT)

### Body

I built Riftcheck to catch a class of bugs that pytest can't: the ones that only exist when AI agents talk to each other.
...
```bash
pip install riftcheck
riftcheck init .          # AST-scans your code, generates scenario + agents
riftcheck run scenario.yaml --view   # runs N times, opens trace viewer
```

`riftcheck init` detects agents from CrewAI, LangGraph, AutoGen, or plain Python functions. No LLM calls — it's pure AST scanning. It generates a scenario.yaml with fault injection (message corruption, hallucinations, contradictions, drops) and property assertions.
...
### Title
Show HN: Riftcheck – Test how your AI agents interact, not just how they work alone
...
I built Riftcheck to catch these. It's a pytest-style framework, but instead of testing functions in isolation, it tests the conversation between agents.
...
`riftcheck init` AST-scans your Python code — no LLM calls. It detects agent classes/functions from CrewAI, LangGraph, AutoGen, or plain Python, extracts their roles and topology, and generates a scenario.yaml with fault injection and property assertions.
...
### Title
Show HN: Riftcheck – Catches bugs that only exist when AI agents talk to each other
...
That's what Riftcheck tests. It simulates adversarial multi-agent interactions and checks properties against the full conversation trace.

```bash
pip install riftcheck
riftcheck init .          # AST-scans your code, generates test scenario
riftcheck run scenario.yaml --view
```
...
**How:** `riftcheck init` does pure AST scanning (no LLM) on your CrewAI / LangGraph / AutoGen / plain Python code. It detects agent classes, extracts roles and topology, and generates a scenario.yaml with fault injection and property assertions. The engine routes all communication — agents never call each other directly. Faults intercept messages between send and receive.
...
Riftcheck tests the conversation between agents, not the agents themselves. It's a pytest-style framework for multi-agent interaction testing.

```bash
pip install riftcheck
riftcheck init .          # AST-scans your code, no LLM
riftcheck run scenario.yaml --view
```

`riftcheck init` detects agents from CrewAI, LangGraph, AutoGen, or plain Python functions using AST scanning. It generates a scenario.yaml with fault injection (corruption, contradictions, hallucinations, message drops) and property assertions — ready to run.
...
- If someone asks about the research → MAST (NeurIPS 2025) identified 14 failure modes in multi-agent systems; Riftcheck maps 16 property checkers to that taxonomy
